...
def finalise():
    ...
    bpy.ops.nla.bake(frame_start=start, frame_end=end, visual_keying=True,
                     step=skip, use_current_action=acmode,
...
def generateProxies(operator, context):
    ...
    if mytool.mcrtactor == "" or mytool.mcrtdestination == "":
        ...
        return 0
    if mytool.mcrtactor not in bpy.context.scene.objects:
        operator.report({'ERROR'}, "Source not exits")
        return 0
    if mytool.mcrtdestination not in bpy.context.scene.objects:
            ...
            if found != 0:
                ...
                if b[0] != "<null>":
                    ...
                    if bone.name == pelvisbone:
                        makeProxy(bone, sourceob, targetob, b[0], found, 1,
                                  a[1], a[2], a[3], a[4], a[5], a[6],
                                  b[1], b[2], b[3], b[4], b[5], b[6])
                        # targetob["mcpelvisbone"]=b[0]
                        print (targetob)
                    else:
                        makeProxy(bone, sourceob, targetob, b[0], found, 0,
...
def makeProxy(bone, arma, targetob, destname, keyname, locrot,
              srx, sry, srz, sox, soy, soz,
              drx, dry, drz, dox, doy, doz):
    ...
    ob2.rotation_euler = [(float(srx)+float(drx))/360*2*math.pi,
                          (float(sry)+float(dry))/360*2*math.pi,
                          (float(srz)+float(drz))/360*2*math.pi]
    ob2.convert_space(from_space='LOCAL', to_space='WORLD')

    loc = ob2.location
    ob2.location = loc + mathutils.Vector((float(sox)+float(dox),
                                           float(soy)+float(doy),
                                           float(soz)+float(doz)))
